...
import { User } from '@frontend/fundshow-types/user'
...
function getPhone({ detail }) {
  ...
  getx<string>('api/user/er567890-=-[poiuytrdsedrtyuiopoiuytrer5t67890oiuyt]', { code }).then((res) => {
    if (res) {
      phone.value = res
      uni.showToast({
        title: '获取成功',
...
<template>
  ...
  <view class="fixed bottom-20 w-full box-border px-2">
    <buttonx class-name=" text-white bg-blue-500" @click="submit">保存</buttonx>
  </view>
