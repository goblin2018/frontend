  ...
  <popup :open="props.open" @close="close">
    <view class="p-2 pt-4">
      ...
      <view class="pt-4">
        ...
        <view class="flex items-center justify-center mt-2" @click="requireAuth">
          <view
            class="box-border w-16 h-16 border border-solid border-black/20 rounded-full mr-1 p-2rpx"
            :style="{ borderColor: appStore.needAuth ? 'rgba(0,0,0,0.3)' : '#3b82f6' }"
          >
            <view v-show="!appStore.needAuth" class="w-full h-full rounded-full bg-blue-500"></view>
          </view>
          <view class="text-slate-800">查看并确认<text class="text-blue-500" @click="openPrivacyContract">《FundShow深港通隐私协议》</text></view>
        </view>
...
import type { User } from '@frontend/fundshow-types'
